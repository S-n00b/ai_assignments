...
from .infrastructure import (
    InfrastructureOrchestrator, InfrastructureConfig, ModelServingConfig,
    InfrastructureProvider, DeploymentEnvironment, InfrastructureStatus
)
...
class ModelLifecycleManager:
    ...
    def __init__(self, registry_path: str = "./model_registry"):
        ...
        self.infrastructure_config = InfrastructureConfig(
            ...
            region="us-west-2",
            cluster_name="lenovo-aaitc-cluster",
            node_count=3,
            node_type="t3.large"
        )
        self.infrastructure_orchestrator = InfrastructureOrchestrator(self.infrastructure_config)
    ...
    async def deploy_infrastructure(self, config: Optional[InfrastructureConfig] = None) -> bool:
        """
        Deploy complete infrastructure stack for model serving.
        
        Args:
            config: Optional infrastructure configuration. Uses default if not provided.
        ...
        try:
            if config:
                ...
                self.infrastructure_orchestrator = InfrastructureOrchestrator(config)
            
            logger.info("Starting infrastructure deployment")
            success = await self.infrastructure_orchestrator.deploy_complete_infrastructure()
            ...
            if success:
                ...
                self.deployment_history.append({
                    ...
                    "config": asdict(self.infrastructure_config),
                    "status": "success"
                })
            else:
    ...
    async def deploy_model_serving(self, model_id: str, version: str, serving_platform: str = "bentoml") -> bool:
        """
        ...
        Args:
            ...
            serving_platform: Serving platform ("bentoml", "ollama", "torchserve")
        ...
        try:
            ...
            logger.info(f"Deploying model serving: {model_id} v{version} on {serving_platform}")
            success = await self.infrastructure_orchestrator.deploy_model_serving(serving_config)
            ...
            if success:
                ...
                logger.info(f"Model serving deployed successfully: {model_id} v{version}")
            else:
                logger.error(f"Model serving deployment failed: {model_id} v{version}")
                self.deployment_history.append({
                    "timestamp": datetime.now().isoformat(),
                    "type": "model_serving_deployment",
                    "model_id": model_id,
    ...
    async def get_infrastructure_status(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing infrastructure status information.
        """
        try:
            status = await self.infrastructure_orchestrator.get_infrastructure_status()
            return status
    ...
    async def scale_model_serving(self, model_id: str, replicas: int) -> bool:
        """
        ...
        """
        try:
            logger.info(f"Scaling model serving: {model_id} to {replicas} replicas")
            
            # Use Kubernetes manager to scale deployment
            success = await self.infrastructure_orchestrator.kubernetes.scale_deployment(
                f"{model_id}-deployment", replicas
            )
            
            if success:
                logger.info(f"Model serving scaled successfully: {model_id}")
                self.deployment_history.append({
                    "timestamp": datetime.now().isoformat(),
                    "type": "model_serving_scale",
                    "model_id": model_id,
                    "replicas": replicas,
                    "status": "success"
                })
            else:
                logger.error(f"Model serving scaling failed: {model_id}")
                self.deployment_history.append({
                    "timestamp": datetime.now().isoformat(),
                    "type": "model_serving_scale",
                    "model_id": model_id,
            ...
            logger.error(f"Error scaling model serving: {e}")
            self.deployment_history.append({
                "timestamp": datetime.now().isoformat(),
                "type": "model_serving_scale",
                "model_id": model_id,
    ...
    async def destroy_infrastructure(self) -> bool:
        ...
        try:
            ...
            success = await self.infrastructure_orchestrator.destroy_infrastructure()
            
            if success:
                logger.info("Infrastructure destroyed successfully")
                self.deployment_history.append({
                    "timestamp": datetime.now().isoformat(),
                    "type": "infrastructure_destruction",
                    "status": "success"
                })
            else:
